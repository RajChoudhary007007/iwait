...
exports.emailverify = (req, res) => {
  ...
  var verify = true;
  db.query(
    `UPDATE user SET email_verify='1' WHERE email='${email}'`,
    function (error, results, fields) {
      if (error) {
        return res.status(400).send({
          status: "false",
  ...
  db.query(
    ...
    function (error, results, fields) {
      if (results) {
        ...
        let updateresults;
        db.query(
          "SELECT * FROM userotp WHERE user_id = ? ",
          [user_id],
          function (error, otpresults, fields) {
            if (otpresults.length > 0) {
              db.query(
                `UPDATE userotp SET otp="${otp}" WHERE user_id = "${user_id}"`,
                function (error, updateresult, fields) {
    ...
    function (error, otpresults, fields) {
      ...
      if (expiretime < 5) {
        if (otpresults[0].otp == req.body.otp) {
          ...
          db.query(
            `UPDATE userotp SET otp="${otp}" WHERE user_id = "${id}"`,
            function (error, updateresult, fields) {}
          );
          return res.status(200).send({
            status: "true",
